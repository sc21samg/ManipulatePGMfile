...
/* argv[2]: n factor               */
/* argv[3]: output file name  template */
...
int main(int argc, char **argv)
{
	...
	//write out the header
	 pgmWriteHeader(theImage, argv[3], outputFile);
		...
		if (theImage->magic_number[1] == '2')
		{
			...
			pgmWriteAsciiData(theImage, argv[3], outputFile);
	...
	else
	{
		...
		if (theImage->magic_number[1] == '5')
		{
			...
			pgmWriteBinaryData(theImage, argv[3], outputFile);
			//print message for successful reduced file
			printf("TILED\n");
		}
	}
